...
@app.route("/",methods=["GET"])    --->methods for frontend linking
def home():
...
def ai_agent_router():
    ...
    cmd_raw=d.get("command") or d.get("text_commend")
    cmd=cmd_raw.strip().lower()   ---->strip is uesd remove unwanted space
    ...
    if "youtube" in cmd:
        ...
        for p in patterns:
            q=q.replace(p,"")
        q=q.strip()         ---->removing unwanted space
        vid=get_vid(q)
        if vid:
        ...
        clean_cmd=re.sub(r'\b(com(and|mand)?)\b','com',clean_com)
